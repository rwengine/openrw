...
void AIGraph::createPathNodes(const glm::vec3& position, const glm::quat& rotation, PathData& path)
{
  ...
  for (size_t n = 0; n < path.nodes.size(); ++n) {
    ...
    if (ainode == nullptr) {
      ...
      ainode->type =
          (path.type == PathData::PATH_PED ? AIGraphNode::Pedestrian : AIGraphNode::Vehicle);
      ...
      if (ainode->external) {
        ...
        if (gridcoord.x < 0 || gridcoord.y < 0 || gridcoord.x >= WORLD_GRID_WIDTH ||
            gridcoord.y >= WORLD_GRID_WIDTH) {
          std::cout << "Warning: Node outside of grid at coord " << gridcoord.x << " "
                    << gridcoord.y << std::endl;
        }
...
void AIGraph::gatherExternalNodesNear(const glm::vec3& center, const float radius,
