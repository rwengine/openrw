...
TrafficDirector::TrafficDirector(AIGraph* g, GameWorld* w)
    : graph(g), world(w), pedDensity(1.f), carDensity(1.f), maximumPedestrians(20), maximumCars(10)
{
}

std::vector<AIGraphNode*> TrafficDirector::findAvailableNodes(AIGraphNode::NodeType type,
...
std::vector<GameObject*> TrafficDirector::populateNearby(const ViewCamera& camera, float radius,
                                                         int maxSpawn)
{
  ...
  for (auto& gen : world->state->vehicleGenerators) {
    ...
    if (dist2 < radius * radius) {
      ...
      if (dist2 <= halfRadius2 && camera.frustum.intersects(position, 1.f)) {
        if (!gen.alwaysSpawn) {
  ...
  for (AIGraphNode* spawn : available) {
    ...
    auto ped =
        world->createPedestrian(validPeds[d(re)], spawn->position + glm::vec3(0.f, 0.f, 1.f));
