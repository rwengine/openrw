...
bool ChaseKeyframe::load(const std::string &filePath, std::vector<ChaseKeyframe> &frames)
{
  ...
  for (size_t i = 0; i < recordCount; ++i) {
    ...
    glm::vec3 velocity{
        rec.velocity[0] / 16383.5f, rec.velocity[1] / 16383.5f, rec.velocity[2] / 16383.5f,
    };
    glm::vec3 right{
    ...
    frames.push_back({velocity, rec.steering, rec.driving, rec.braking, !!rec.handbrake,
                      rec.position, glm::quat_cast(rotation)});
  }
...
GameObject *ChaseCoordinator::getChaseVehicle(int index) { return chaseVehicles.at(index).object; }
...
void ChaseCoordinator::start() { chaseTime = 0.f; }
