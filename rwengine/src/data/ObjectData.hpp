...
struct ObjectData : public ObjectInformation {
  ...
  enum {
    ...
    DRAW_LAST = 1 << 2,    /// Model is transparent. Render this object after all opaque objects,
                           /// allowing transparencies of other objects to be visible through this
                           /// object.
    ADDITIVE = 1 << 3,     /// Render with additive blending. Previous flag must be enabled too.
    IS_SUBWAY = 1 << 4,    /// Model is a tunnel, i.e. set the object as invisible unless the player
                           /// enters cull zone flag 128. This flag works only with static models.
    IGNORE_LIGHTING = 1 << 5,   /// Don't use static lighting, we want dynamic if it's possible.
    NO_ZBUFFER_WRITE = 1 << 6,  /// Model is a shadow. Disable writing to z-buffer when rendering
                                /// it, allowing transparencies of other objects, shadows, and
                                /// lights to be visible through this object. (Not implemented in
                                /// the PS2 version)
  };
