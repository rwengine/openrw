...
struct VehicleGenerator {
  ...
  */
  ...
  VehicleGenerator(const glm::vec3& position_, float heading_, int modelID_, int colourFG_,
                   int colourBG_, bool alwaysSpawn_, short alarmThreshold_, short lockedThreshold_,
                   int minDelay_, int maxDelay_, int lastSpawnTime_, int remainingSpawns_)
      : position(position_),
        heading(heading_),
        vehicleID(modelID_),
        colourFG(colourFG_),
