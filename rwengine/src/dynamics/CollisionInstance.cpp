...
bool CollisionInstance::createPhysicsBody(GameObject* object, const std::string& modelName,
                                          ...
                                          VehicleHandlingInfo* handling)
{
  ...
  if (phyit != object->engine->data->collisions.end()) {
    ...
    float colMin = std::numeric_limits<float>::max(), colMax = std::numeric_limits<float>::lowest();
    ...
    if (physInst.vertices.size() > 0 && physInst.indices.size() >= 3) {
      m_vertArray = new btTriangleIndexVertexArray(
          physInst.indices.size() / 3, (int*)physInst.indices.data(), sizeof(uint32_t) * 3,
          physInst.vertices.size(), &(physInst.vertices[0].x), sizeof(glm::vec3));
