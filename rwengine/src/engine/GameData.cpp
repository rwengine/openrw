...
void GameData::loadIMG(const std::string& name) { index.indexArchive(datpath + name); }

void GameData::loadIPL(const std::string& name) { iplLocations.insert({name, name}); }
    ...
    if (line.substr(0, 1) == "#") {  // Comment
      ...
      if (currentSection == COL) {
        ...
        if (std::getline(ss, r, ',') && std::getline(ss, g, ',') && std::getline(ss, b)) {
          vehicleColours.push_back(glm::u8vec3(atoi(r.c_str()), atoi(g.c_str()), atoi(b.c_str())));
        }
...
void GameData::loadWeather(const std::string& path) { weatherLoader.load(path); }
...
void GameData::loadDFF(const std::string& name, bool async)
{
  ...
  auto job =
      new BackgroundLoaderJob<Model, LoaderDFF>{workContext, &this->index, name, models[realname]};
