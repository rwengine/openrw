    ...
    : logger(log), data(dat), randomEngine(rand()), _work(work), paused(false)
{
  ...
  dynamicsWorld =
      new btDiscreteDynamicsWorld(collisionDispatcher, broadphase, solver, collisionConfig);
  ...
  if (ipll.load(path)) {
    ...
    for (size_t i = 0; i < ipll.m_instances.size(); ++i) {
      ...
      if (!createInstance(inst->id, inst->pos, inst->rot)) {
        logger->error("World",
                      "No object data for instance " + std::to_string(inst->id) + " in " + path);
      }
    }
                                          ...
                                          const glm::quat& rot)
{
  ...
  if (oi) {
    ...
    std::transform(std::begin(texturename), std::end(texturename), std::begin(texturename),
                   tolower);
    ...
    auto instance =
        new InstanceObject(this, pos, rot, m, glm::vec3(1.f, 1.f, 1.f), oi, nullptr, dydata);
...
#include <core/Logger.hpp>
CutsceneObject* GameWorld::createCutsceneObject(const uint16_t id, const glm::vec3& pos,
                                                const glm::quat& rot)
{
...
CharacterObject* GameWorld::createPlayer(const glm::vec3& pos, const glm::quat& rot,
                                         GameObjectID gid)
{
                               ...
                               const glm::vec3& max)
{
  for (AIGraphNode* n : aigraph.nodes) {
    if (n->type == type) {
      if (n->position.x >= min.x && n->position.y >= min.y && n->position.z >= min.z &&
          n->position.x <= max.x && n->position.y <= max.y && n->position.z <= max.z) {
        n->disabled = true;
      }
    }
  }
...
void GameWorld::drawAreaIndicator(AreaIndicatorInfo::AreaIndicatorType type, glm::vec3 position,
                                  glm::vec3 radius)
{
...
VehicleObject* GameWorld::tryToSpawnVehicle(VehicleGenerator& gen)
{
  ...
  for (auto& v : vehiclePool.objects) {
    if (glm::distance2(position, v.second->getPosition()) < kMinClearRadius * kMinClearRadius) {
      return nullptr;
    }
  }
