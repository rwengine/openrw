...
bool SaveGame::loadGame(GameState& state, const std::string& file)
{
  ...
  if (fread(state.script->getGlobals(), sizeof(SCMByte), scriptVarCount, loadFile) !=
      scriptVarCount) {
...
#if RW_DEBUG
    std::cout << "Player Health: " << ped.info.health << " (" << ped.info.armour << ")"
              << std::endl;
    std::cout << "Player model: " << ped.modelName << std::endl;
    for (int w = 0; w < 13; ++w) {
      auto& wep = players[p].info.weapons[w];
      std::cout << "ID " << wep.weaponId << " " << wep.inClip << " " << wep.totalBullets
                << std::endl;
    }
#endif
  }
...
#if RW_DEBUG
    std::cout << " v " << veh.modelId << " " << veh.state.position.x << " " << veh.state.position.y
              << " " << veh.state.position.z << std::endl;
#endif
  }
  ...
  for (size_t v = 0; v < boatCount; ++v) {
    ...
    READ_VALUE(veh.state)
#if RW_DEBUG
    std::cout << " b " << veh.modelId << " " << veh.state.position.x << " " << veh.state.position.y
              << " " << veh.state.position.z << std::endl;
#endif
  }
  ...
  for (size_t o = 0; o < objectCount; ++o) {
    ...
    glm::vec3 forward =
        glm::normalize(glm::vec3(obj.rotation[3], obj.rotation[4], obj.rotation[5]));
  ...
  for (size_t c = 0; c < craneData.numCranes; ++c) {
    Block6Crane& crane = craneData.cranes[c];
    std::cout << "pickup " << crane.x1Pickup << " " << crane.y1Pickup << " " << crane.x2Pickup
              << " " << crane.y2Pickup << std::endl;
  ...
  for (int c = 0; c < 336; ++c) {
    ...
    std::cout << " " << uint16_t(p.type) << " " << p.position.x << " " << p.position.y << " "
  ...
  for (size_t p = 0; p < phoneData.numPhones; ++p) {
    Block8Phone& phone = phones[p];
    std::cout << " " << uint16_t(phone.state) << " " << phone.position.x << " " << phone.position.y
              << " " << phone.position.z << std::endl;
  }
#endif
...
#if RW_DEBUG
  std::cout << "Hospitals: " << restartData.numHospitals << " police: " << restartData.numPolice
            << std::endl;
  ...
  for (int g = 0; g < 9; ++g) {
    ...
    std::cout << " " << p.carModelId << " " << p.weaponPrimary << " " << p.weaponSecondary
              << std::endl;
  }
#endif
  ...
  for (size_t g = 0; g < carGeneratorData.generatorCount; ++g) {
    ...
    std::cout << " " << gen.modelId << " " << gen.position.x << " " << gen.position.y << " "
              << gen.position.z << std::endl;
  }
#endif
...
#if RW_DEBUG
  std::cout << "Player money: " << state.playerInfo.money << " (" << state.playerInfo.displayedMoney
            << ")" << std::endl;
#endif
  ...
  state.scriptOnMissionFlag =
      (unsigned int*)state.script->getGlobals() + (size_t)scriptData.onMissionOffset;
  ...
  for (int c = 0; c < 18; ++c) {
    ...
    glm::quat rotation(glm::mat3(glm::cross(car.rotation, glm::vec3(0.f, 0.f, 1.f)), car.rotation,
  ...
  for (unsigned g = 0; g < carGenerators.size(); ++g) {
    ...
    state.vehicleGenerators.emplace_back(
        gen.position, gen.angle, gen.modelId, gen.colourFG, gen.colourBG, gen.force,
        gen.alarmChance, gen.lockedChance, gen.minDelay, gen.maxDelay, gen.timestamp,
