...
void WeaponItem::fireHitscan(CharacterObject* owner)
{
  ...
  auto farTarget =
      owner->getPosition() + owner->getRotation() * glm::vec3(0.f, _wepData->hitRange, 0.f);
...
void WeaponItem::fireProjectile(CharacterObject* owner)
{
  ...
  float throwTime =
      owner->engine->getGameTime() - owner->getCurrentState().primaryStartTime / 1000.f;
  float forceFactor = throwTime;
  if (owner->getCurrentState().primaryEndTime >= owner->getCurrentState().primaryStartTime) {
    uint32_t heldTime =
        owner->getCurrentState().primaryEndTime - owner->getCurrentState().primaryStartTime;
    forceFactor = (heldTime) / 1000.f;
  }
  forceFactor = std::max(0.1f, forceFactor / throwTime);

  auto projectile =
      new ProjectileObject(owner->engine, fireOrigin,
                           {pt, direction,
...
void WeaponItem::secondary(CharacterObject* owner) { RW_UNUSED(owner); }
...
bool WeaponItem::isFiring(CharacterObject* owner) { return owner->getCurrentState().primaryActive; }
