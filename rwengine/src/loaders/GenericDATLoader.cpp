...
void GenericDATLoader::loadWeapons(const std::string& name, WeaponDataPtrs& weaponData)
{
  ...
  if (dfile.is_open()) {
    ...
    while (std::getline(dfile, linebuffer)) {
      ...
      if (std::find_if(data->name.begin(), data->name.end(), ::isalnum) == std::end(data->name)) {
        continue;
      }
      ...
      ss >> data->animation1;
      std::transform(data->animation1.begin(), data->animation1.end(), data->animation1.begin(),
                     ::tolower);
      ss >> data->animation2;
      std::transform(data->animation2.begin(), data->animation2.end(), data->animation2.begin(),
                     ::tolower);
  ...
  if (hndFile.is_open()) {
    ...
    while (std::getline(hndFile, lineBuff)) {
      ...
      if (mit == vehicleData.end()) {
        vehicleData.insert({info.ID, VehicleInfoHandle(new VehicleInfo{info, {}, {}})});
      } else {
