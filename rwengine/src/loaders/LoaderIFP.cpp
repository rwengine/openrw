...
bool findKeyframes(float t, AnimationBone* bone, AnimationKeyframe& f1, AnimationKeyframe& f2,
                   float& alpha)
{
...
AnimationKeyframe AnimationBone::getInterpolatedKeyframe(float time)
{
  ...
  if (findKeyframes(time, this, f1, f2, alpha)) {
    return {glm::normalize(glm::slerp(f1.rotation, f2.rotation, alpha)),
            glm::mix(f1.position, f2.position, alpha), glm::mix(f1.scale, f2.scale, alpha), time,
            std::max(f1.id, f2.id)};
  }
      ...
      if (type == "KR00") {
        ...
        for (int d = 0; d < frames->frames; ++d) {
          ...
          bonedata->frames.push_back(
              {q, glm::vec3(0.f, 0.f, 0.f), glm::vec3(1.f, 1.f, 1.f), time, d});
        }
      } else if (type == "KRT0") {
        ...
        for (int d = 0; d < frames->frames; ++d) {
          ...
          time = *read<float>(data, dataI);
          bonedata->frames.push_back({q, p, glm::vec3(1.f, 1.f, 1.f), time, d});
        }
      } else if (type == "KRTS") {
