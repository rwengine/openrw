...
CharacterObject::CharacterObject(GameWorld* engine, const glm::vec3& pos, const glm::quat& rot,
...
glm::vec3 CharacterObject::updateMovementAnimation(float dt)
{
  ...
  if (!isAlive()) {
    ...
    repeat = false;
    if (currentAnim == animations.jump_start && animator->isCompleted(AnimIndexMovement)) {
      movementAnimation = animations.jump_start;
    }
    if (isOnGround()) {
      if (currentAnim != animations.jump_land || !animator->isCompleted(AnimIndexMovement)) {
...
bool CharacterObject::isAlive() const { return currentState.health > 0.f; }
...
VehicleObject* CharacterObject::getCurrentVehicle() const { return currentVehicle; }

size_t CharacterObject::getCurrentSeat() const { return currentSeat; }
...
float CharacterObject::getJumpSpeed() const { return jumpSpeed; }
...
void CharacterObject::setJumpSpeed(float speed) { jumpSpeed = speed; }
...
void CharacterObject::setActiveItem(int slot) { currentState.currentWeapon = slot; }
...
void CharacterObject::removeFromInventory(int slot) { currentState.weapons[slot].weaponId = 0; }
