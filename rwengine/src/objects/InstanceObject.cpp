...
InstanceObject::InstanceObject(GameWorld* engine, const glm::vec3& pos, const glm::quat& rot,
                               const ModelRef& model, const glm::vec3& scale,
                               std::shared_ptr<ObjectData> obj, InstanceObject* lod,
  ...
  if (dynamics && body) {
    ...
    if (inWater) {
      ...
      if (wi != NO_WATER_INDEX) {
        ...
        if (ws.z <= h) {
          /*if( dynamics->uprootForce > 0.f && (body->body->getCollisionFlags() &
          ...
          float F = WATER_BUOYANCY_K * x +
                    -WATER_BUOYANCY_C * body->getBulletBody()->getLinearVelocity().z();
          btVector3 forcePos = btVector3(0.f, 0.f, 2.f)
                                   .rotate(body->getBulletBody()->getOrientation().getAxis(),
                                           body->getBulletBody()->getOrientation().getAngle());
          body->getBulletBody()->applyForce(btVector3(0.f, 0.f, F), forcePos);
        }
      }
    }
  }
...
bool InstanceObject::takeDamage(const GameObject::DamageInfo& dmg)
{
  ...
  if (dynamics) {
    ...
    if (dmg.impulse >= dynamics->uprootForce &&
        (body->getBulletBody()->getCollisionFlags() & btRigidBody::CF_STATIC_OBJECT) != 0) {
      _enablePhysics = true;
    }
  }
  if (smash) {
