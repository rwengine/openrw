...
VehicleObject::VehicleObject(GameWorld* engine, const glm::vec3& pos, const glm::quat& rot,
                             const ModelRef& model, VehicleDataHandle data, VehicleInfoHandle info,
      ...
      physVehicle(nullptr)
{
  ...
  if (collision->createPhysicsBody(this, data->modelName, nullptr, &info->handling)) {
    ...
    for (size_t w = 0; w < info->wheels.size(); ++w) {
      ...
      btVector3 connection(info->wheels[w].position.x, info->wheels[w].position.y,
                           info->wheels[w].position.z + heightOffset);
      bool front = connection.y() > 0;
      btWheelInfo& wi =
          physVehicle->addWheel(connection, btVector3(0.f, 0.f, -1.f), btVector3(1.f, 0.f, 0.f),
      ...
      wi.m_frictionSlip =
          halfFriction +
          halfFriction * (front ? info->handling.tractionBias : 1.f - info->handling.tractionBias);
    }
  ...
  if (physVehicle) {
    ...
    for (int w = 0; w < physVehicle->getNumWheels(); ++w) {
      ...
      float brakeReal =
          ...
          (wi.m_bIsFrontWheel ? info->handling.brakeBias : 1.f - info->handling.brakeBias);
      ...
      if (wi.m_bIsFrontWheel) {
        ...
        physVehicle->setSteeringValue(
            std::min(info->handling.steeringLock * (3.141f / 180.f), std::abs(steerAngle)) * sign,
            w);
    ...
    if (vehicle->type == VehicleData::BOAT) {
      if (isInWater()) {
        ...
        collision->getBulletBody()->applyForce(rudderVector * engineForce * 100.f, rudderPoint);
...
void VehicleObject::setSteeringAngle(float a) { steerAngle = a; }

float VehicleObject::getSteeringAngle() const { return steerAngle; }

void VehicleObject::setThrottle(float t) { throttle = t; }

float VehicleObject::getThrottle() const { return throttle; }

void VehicleObject::setBraking(float b) { brake = b; }

float VehicleObject::getBraking() const { return brake; }

void VehicleObject::setHandbraking(bool hb) { handbrake = hb; }

bool VehicleObject::getHandbraking() const { return handbrake; }
...
bool VehicleObject::canOccupantExit() const { return getVelocity() <= kVehicleMaxExitVelocity; }
...
{
  auto it = std::find_if(f->getChildren().begin(), f->getChildren().end(), [&](ModelFrame* c) {
...
void VehicleObject::registerPart(ModelFrame* mf)
{
  ...
  dynamicParts.insert(
      {mf->getName(), {mf, normal, damage, nullptr, nullptr, false, 0.f, 0.f, 0.f}});
}
...
void VehicleObject::createObjectHinge(Part* part)
{
  ...
  auto hinge = new btHingeConstraint(*collision->getBulletBody(), *subObject, tr.getOrigin(),
                                     hingePosition, hingeAxis, hingeAxis);
