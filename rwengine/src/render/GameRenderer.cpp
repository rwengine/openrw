...
constexpr uint32_t kMissingTextureBytes[] = {
    0xFF0000FF, 0xFFFF00FF, 0xFF0000FF, 0xFFFF00FF, 0xFFFF00FF, 0xFF0000FF, 0xFFFF00FF, 0xFF0000FF,
    0xFF0000FF, 0xFFFF00FF, 0xFF0000FF, 0xFFFF00FF, 0xFFFF00FF, 0xFF0000FF, 0xFFFF00FF, 0xFF0000FF,
};
      ...
      text(this)
{
  ...
  worldProg = renderer->createShader(GameShaders::WorldObject::VertexShader,
  ...
  particleProg = renderer->createShader(GameShaders::WorldObject::VertexShader,
  ...
  skyProg =
      renderer->createShader(GameShaders::Sky::VertexShader, GameShaders::Sky::FragmentShader);

  renderer->setProgramBlockBinding(skyProg, "SceneData", 1);

  postProg = renderer->createShader(GameShaders::DefaultPostProcess::VertexShader,
  ...
  for (size_t r = 0, i = 0; r < rows; ++r) {
    for (size_t s = 0; s < segments; ++s) {
      skydomeVerts[i++].position =
          glm::vec3(cos(2.f * M_PI * s * S) * cos(M_PI_2 * r * R),
                    sin(2.f * M_PI * s * S) * cos(M_PI_2 * r * R), sin(M_PI_2 * r * R));
    }
  }
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(GLuint) * skydomeIndBuff.size(),
               skydomeIndBuff.data(), GL_STATIC_DRAW);
  ...
  particleGeom.uploadVertices<ParticleVert>({{0.5f, 0.5f, 1.f, 1.f, 1.f, 1.f, 1.f},
  ...
  for (int s = 0; s < cylsegments; ++s) {
    ...
    cylverts.push_back(
        {glm::vec3(p0, 2.f), glm::vec3(), glm::vec2(0.45f, 0.6f), glm::u8vec4(255, 255, 255, 50)});
    cylverts.push_back({glm::vec3(p0, -1.f), glm::vec3(), glm::vec2(0.45f, 0.4f),
                        glm::u8vec4(255, 255, 255, 150)});
    cylverts.push_back(
        {glm::vec3(p1, 2.f), glm::vec3(), glm::vec2(0.55f, 0.6f), glm::u8vec4(255, 255, 255, 50)});

    cylverts.push_back({glm::vec3(p0, -1.f), glm::vec3(), glm::vec2(0.45f, 0.4f),
                        glm::u8vec4(255, 255, 255, 150)});
    cylverts.push_back({glm::vec3(p1, -1.f), glm::vec3(), glm::vec2(0.55f, 0.4f),
                        glm::u8vec4(255, 255, 255, 150)});
    cylverts.push_back(
        {glm::vec3(p1, 2.f), glm::vec3(), glm::vec2(0.55f, 0.6f), glm::u8vec4(255, 255, 255, 50)});
  }
...
GameRenderer::~GameRenderer() { glDeleteFramebuffers(1, &framebufferName); }

float mix(uint8_t a, uint8_t b, float num) { return a + (b - a) * num; }
...
void GameRenderer::renderWorld(GameWorld* world, const ViewCamera& camera, float alpha)
{
  ...
  auto weatherID =
      static_cast<WeatherLoader::WeatherCondition>(world->state->basic.nextWeather * 24);
  ...
  Renderer::SceneUniformData sceneParams{
      proj, view, glm::vec4{ambient, 0.0f}, glm::vec4{dynamic, 0.0f}, glm::vec4(skyBottom, 1.f),
      glm::vec4(camera.position, 0.f), weather.fogStart, _camera.frustum.far};
  ...
  if (cullOverride) {
    cullingCamera.frustum.update(cullingCamera.frustum.projection() * cullingCamera.getView());
  }
  ...
  ObjectRenderer objectRenderer(_renderWorld, (cullOverride ? cullingCamera : _camera),
  ...
  std::sort(renderList.begin(), renderList.end(),
            [](const Renderer::RenderInstruction& a, const Renderer::RenderInstruction& b) {
...
void GameRenderer::renderGeometry(Model* model, size_t g, const glm::mat4& modelMatrix,
                                  float opacity, GameObject* object)
{
...
void GameRenderer::renderEffects(GameWorld* world)
{
  ...
  std::sort(effects.begin(), effects.end(), [&](const VisualFX* a, const VisualFX* b) {
...
bool GameRenderer::renderFrame(Model* m, ModelFrame* f, const glm::mat4& matrix, GameObject* object,
                               float opacity, bool queueTransparent)
{
...
void GameRenderer::renderPaths()
{
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * carlines.size(), &(carlines[0]),
  GL_STREAM_DRAW);
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * pedlines.size(), &(pedlines[0]),
  GL_STREAM_DRAW);
  glVertexAttribPointer(posAttrib, 3, GL_FLOAT, GL_FALSE, 0, 0);
