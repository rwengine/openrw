...
MapRenderer::MapRenderer(Renderer* renderer, GameData* _data) : data(_data), renderer(renderer)
{
  rectGeom.uploadVertices<VertexP2>({{-.5f, .5f}, {.5f, .5f}, {-.5f, -.5f}, {.5f, -.5f}});
  ...
  for (int v = 0; v < 181; ++v) {
    circleVerts.push_back({0.5f * glm::cos(2 * (v / 180.f) * glm::pi<float>()),
...
void MapRenderer::drawBlip(const glm::vec2& coord, const glm::mat4& view, const MapInfo& mi,
                           const std::string& texture, float heading, float size)
{
