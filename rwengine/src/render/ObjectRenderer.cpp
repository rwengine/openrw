...
void ObjectRenderer::renderGeometry(Model* model, size_t g, const glm::mat4& modelMatrix,
                                    float opacity, GameObject* object, RenderList& outList)
{
  for (size_t sg = 0; sg < model->geometries[g]->subgeom.size(); ++sg) {
    ...
    if (model->geometries[g]->materials.size() > subgeom.material) {
      ...
      if ((model->geometries[g]->flags & RW::BSGeometry::ModuleMaterialColor) ==
    ...
    float depth =
        (distance - m_camera.frustum.near) / (m_camera.frustum.far - m_camera.frustum.near);
    outList.emplace_back(createKey(isTransparent, depth * depth, dp.textures), modelMatrix,
                         &model->geometries[g]->dbuff, dp);
  }
}
bool ObjectRenderer::renderFrame(Model* m, ModelFrame* f, const glm::mat4& matrix,
...
void ObjectRenderer::renderItem(InventoryItem* item, const glm::mat4& modelMatrix,
                                RenderList& outList)
{
  // srhand
  ...
  if (weapons && weapons->resource) {
    ...
    if (itemModel) {
      renderFrame(weapons->resource, itemModel, modelMatrix * matrix, nullptr, 1.f, outList);
    }
  }
}

void ObjectRenderer::renderInstance(InstanceObject* instance, RenderList& outList)
{
  ...
  if (model) {
    ...
    renderFrame(model, frame, matrixModel * glm::inverse(frame->getTransform()), instance, 1.f,
                outList);
  }
  if (fadingModel) {
    if (opacity >= 0.01f) {
      fadingFrame = fadingFrame ? fadingFrame : fadingModel->frames[0];
      renderFrame(fadingModel, fadingFrame,
                  fadingMatrix * glm::inverse(fadingFrame->getTransform()), instance, opacity,
                  outList);
    }
  }
}

void ObjectRenderer::renderCharacter(CharacterObject* pedestrian, RenderList& outList)
{
  ...
  renderFrame(pedestrian->model->resource, root->getChildren()[0], matrixModel, pedestrian, 1.f,
              outList);
  ...
  if (pedestrian->getActiveItem()) {
    ...
    renderItem(pedestrian->getActiveItem(), matrixModel * localMatrix, outList);
  }
}

void ObjectRenderer::renderWheel(VehicleObject* vehicle, Model* model, const glm::mat4& matrix,
...
void ObjectRenderer::renderVehicle(VehicleObject* vehicle, RenderList& outList)
{
  ...
  renderFrame(vehicle->model->resource, vehicle->model->resource->frames[0], matrixModel, vehicle,
              1.f, outList);
  ...
  for (size_t w = 0; w < vehicle->info->wheels.size(); ++w) {
    ...
    if (woi) {
      ...
      if (wheelModel) {
        ...
        btMatrix3x3 basis(right[0], fwd[0], up[0], right[1], fwd[1], up[1], right[2], fwd[2],
                          up[2]);
...
void ObjectRenderer::renderPickup(PickupObject* pickup, RenderList& outList)
{
  ...
  if (itemModel) {
    auto matrix = glm::inverse(itemModel->getTransform());
    renderFrame(model, itemModel, modelMatrix * matrix, pickup, 1.f, outList);
  }
}

void ObjectRenderer::renderCutsceneObject(CutsceneObject* cutscene, RenderList& outList)
{
  ...
  glm::mat4 matrixModel;
  auto cutsceneOffset =
      m_world->state->currentCutscene->meta.sceneOffset + glm::vec3(0.f, 0.f, 1.f);
  ...
  if (cutscene->getParentActor()) {
    ...
    while (boneframe) {
      localMatrix =
          cutscene->getParentActor()->skeleton->getMatrix(boneframe->getIndex()) * localMatrix;
  ...
  if (cutscene->getParentActor()) {
    ...
    renderFrame(model, model->frames[0], matrixModel * align, cutscene, 1.f, outList);
  } else {
    renderFrame(model, model->frames[0], matrixModel, cutscene, 1.f, outList);
  }
}

void ObjectRenderer::renderProjectile(ProjectileObject* projectile, RenderList& outList)
{
  glm::mat4 modelMatrix = projectile->getTimeAdjustedTransform(m_renderAlpha);

  auto odata =
      m_world->data->findObjectType<ObjectData>(projectile->getProjectileInfo().weapon->modelID);
  auto weapons = m_world->data->models["weapons"];
  ...
  if (weapons && weapons->resource) {
    ...
    if (itemModel) {
      renderFrame(weapons->resource, itemModel, modelMatrix * matrix, projectile, 1.f, outList);
    }
  }
}
