...
WaterRenderer::WaterRenderer(GameRenderer* renderer) : waterProg(nullptr)
{
  ...
  waterProg = renderer->getRenderer()->createShader(GameShaders::WaterHQ::VertexShader,
                                                    GameShaders::WaterHQ::FragmentShader);
  maskProg = renderer->getRenderer()->createShader(GameShaders::Mask3D::VertexShader,
...
void WaterRenderer::setWaterTable(float* waterHeights, unsigned int nHeights, uint8_t* tiles,
                                  unsigned int nTiles)
{
  ...
  maskGeom.uploadVertices(vertexData.size(), sizeof(glm::vec3) * vertexData.size(),
                          vertexData.data());
