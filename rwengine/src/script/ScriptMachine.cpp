...
void ScriptMachine::executeThread(SCMThread& t, int msPassed)
{
  ...
  while (t.wakeCounter == 0) {
    ...
    bool isNegatedConditional =
        ((opcode & SCM_NEGATE_CONDITIONAL_MASK) == SCM_NEGATE_CONDITIONAL_MASK);
      ...
      switch (type) {
        ...
        case TGlobal: {
          ...
          if (v >= _file->getGlobalsSize()) {
            state->world->logger->error("SCM", "Global Out of bounds! " + std::to_string(v) + " " +
        ...
        case TInt32:
          ...
          break;
        case TString:
          std::copy(_file->data() + pc, _file->data() + pc + 8, parameters.back().string);
          pc += sizeof(SCMByte) * 8;
          break;
        case TFloat16:
    ...
    if (!sDebugThreadName || strncmp(t.name, sDebugThreadName, 8) == 0) {
      printf("%8s %01x %06x %04x %s", t.name, t.conditionResult, t.programCounter, opcode,
             code.signature.c_str());
    ...
    if (t.conditionCount > 0 &&
        opcode != 0x00D6)  /// @todo add conditional flag to opcodes instead of checking for 0x00D6
    {
      --t.conditionCount;
      if (t.conditionAND) {
    ...
    : _file(file), _ops(ops), state(_state)
{
  ...
  std::copy(_file->data() + offset, _file->data() + offset + size, globalData.begin());
}
...
SCMByte* ScriptMachine::getGlobals() { return globalData.data(); }
