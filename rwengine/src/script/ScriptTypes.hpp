...
using ScriptRGB = glm::i32vec3;
using ScriptRGBA = glm::i32vec4;
...
struct ScriptObjectType {
    ...
    ScriptObjectType(ScriptInt* var, T* object) : m_id(var), m_object(object) {
    }
    ...
    T* operator->() const {
        RW_CHECK(m_object != nullptr,
                 "Dereferencing ScriptObjectType with null instance");
        return m_object;
    }
...
public:
    ...
    const SCMParams& getParameters() const {
        return *parameters;
    }
    SCMThread* getThread() const {
        return thread;
    }
    ScriptMachine* getVM() const {
        return machine;
    }
