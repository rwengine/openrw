...
void game_print_big(const ScriptArguments& args)
{
  ...
  args.getWorld()->state->text.addText<ScreenTextType::Big>(
      ScreenTextEntry::makeBig(id, str, style, time));
}
...
void game_print_now(const ScriptArguments& args)
{
  ...
  args.getWorld()->state->text.addText<ScreenTextType::HighPriority>(
      ScreenTextEntry::makeHighPriority(id, str, time));
}
...
void game_create_vehicle_generator(const ScriptArguments& args)
{
  ...
  VehicleGenerator vg{position, heading, vehicleID, colourFG, colourBG, alwaysSpawn, alarmThreshold,
                      lockedThreshold, minDelay, maxDelay, 0, 0};
...
{
  RW_CHECK(args.getWorld()->state->vehicleGenerators.size() > *args[0].globalInteger,
           "VehicleGenerator out of range");
  if ((int)args.getWorld()->state->vehicleGenerators.size() > *args[0].globalInteger) {
    VehicleGenerator& generator =
        args.getWorld()->state->vehicleGenerators.at(*args[0].globalInteger);
...
void game_override_restart(const ScriptArguments& args)
{
  ...
  args.getWorld()->state->nextRestartLocation =
      glm::vec4(args[0].real, args[1].real, args[2].real, args[3].real);
}
...
{
  std::string id(args[0].string);
  std::string str = ScreenText::format(args.getWorld()->data->texts.text(id), formatValue(args[1]));
  unsigned short style = args[3].integer;
  args.getWorld()->state->text.addText<ScreenTextType::Big>(
      ScreenTextEntry::makeBig(id, str, style, 5000));
}
...
void game_create_garage(const ScriptArguments& args)
{
  ...
  args.getWorld()->state->garages.push_back({min, max, garageType});
...
bool game_is_car_inside_garage(const ScriptArguments& args)
{
  ...
  for (auto& v : gw->vehiclePool.objects) {
    // @todo if this car only accepts mission cars we probably have to filter here / only check for
    // one specific car
...
bool game_special_char_loaded(const ScriptArguments& args)
{
  ...
  if (chartype) {
    ...
    if (modelfind != args.getWorld()->data->models.end() &&
        modelfind->second->resource != nullptr) {
      return true;
    }
  }
...
void game_controller_mode(const ScriptArguments& args) { *args[0].globalInteger = 0; }
...
static const char* sprite_names[] = {
    "",  // 0
    "radar_asuka", "radar_bomb", "radar_cat", "radar_centre", "radar_copcar", "radar_don",
    "radar_eight", "radar_el", "radar_ice", "radar_joey", "radar_kenji", "radar_liz", "radar_luigi",
    "radar_north", "radar_ray", "radar_sal", "radar_save", "radar_spray", "radar_tony",
    "radar_weapon",
};
...
void game_create_cutscene_object(const ScriptArguments& args)
{
  ...
  GameObject* object = args.getWorld()->createCutsceneObject(
...
void game_start_cutscene(const ScriptArguments& args) { args.getWorld()->startCutscene(); }
...
void game_clear_cutscene(const ScriptArguments& args) { args.getWorld()->clearCutscene(); }
...
#define CHASE_VEHICLE(model, x, y, z, hdg, c1, c2, path)                                          \
  {                                                                                               \
    auto vehicle0 = args.getWorld()->createVehicle(                                               \
        model, glm::vec3(x, y, z), glm::angleAxis(glm::radians(hdg), glm::vec3(0.f, 0.f, 1.f)));  \
    vehicle0->setPrimaryColour(c1);                                                               \
    vehicle0->setSecondaryColour(c2);                                                             \
    args.getWorld()->chase.addChaseVehicle(                                                       \
        vehicle0, path, args.getWorld()->data->getDataPath() + "/data/paths/CHASE" #path ".DAT"); \
  }
...
void game_print_big_with_2_numbers(const ScriptArguments& args)
{
  ...
  std::string str =
      ScreenText::format(world->data->texts.text(id), formatValue(args[1]), formatValue(args[2]));
...
void game_clear_area(const ScriptArguments& args)
{
  ...
  /// @todo Do we also have to clear all projectiles + particles *in this area*, even if the bool is
  /// false?

  if (clearParticles) {
    RW_UNUSED(clearParticles);
    RW_UNIMPLEMENTED(
        "game_clear_area(): should clear all particles and projectiles (not limited to area!)");
  }
}
...
void game_display_help(const ScriptArguments& args)
{
  ...
  args.getWorld()->state->text.addText<ScreenTextType::Big>(ScreenTextEntry::makeHelp(id, str));
}
...
GameModule::GameModule() : ScriptModule("Game")
{
  ...
  bindUnimplemented(0x021B, game_set_target_car_for_mission_garage, 2,
                    "Set Target Car for Mission Garage");
  ...
  bindFunction(0x03D4, game_import_garage_contains_needed_car, 2,
               "Import Garage Contains Needed Car");
  ...
  bindUnimplemented(0x0426, game_create_level_transition, 6,
                    "Create Save Cars Between Levels cube");
