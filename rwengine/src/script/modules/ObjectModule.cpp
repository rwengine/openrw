...
bool game_player_in_area_3d(const ScriptArguments& args)
{
  ...
  auto player = character->getPosition();
  if (player.x > min.x && player.y > min.y && player.z > min.z && player.x < max.x &&
      player.y < max.y && player.z < max.z) {
    return true;
  }
  return false;
}
...
bool game_player_in_area_2d_in_vehicle(const ScriptArguments& args)
{
  ...
  if (drawCylinder) {
    ...
    args.getWorld()->drawAreaIndicator(AreaIndicatorInfo::Cylinder,
                                       ground + glm::vec3(0.f, 0.f, 4.5f), glm::vec3(radius, 5.f));
  }
...
bool game_character_stoped_in_volume_in_vehicle(const ScriptArguments& args)
{
  ...
  if (character && character->getCurrentVehicle() != nullptr) {
    ...
    if (pp.x >= min.x && pp.y >= min.y && pp.z >= min.z && pp.x <= max.x && pp.y <= max.y &&
        pp.z <= max.z) {
    ...
    if (drawCylinder) {
      args.getWorld()->drawAreaIndicator(AreaIndicatorInfo::Cylinder, (max + min) / 2.f,
                                         (max - min) / 2.f);
    }
  }
  return false;
...
bool game_vehicle_stopped_in_volume(const ScriptArguments& args)
{
  auto vehicle = static_cast<VehicleObject*>(args.getObject<VehicleObject>(0));

  glm::vec3 vec1(args[1].realValue(), args[2].realValue(), args[3].realValue());
  glm::vec3 vec2(args[4].realValue(), args[5].realValue(), args[6].realValue());
  bool drawCylinder = !!args[7].integerValue();

  glm::vec3 min = glm::min(vec1, vec2);
  glm::vec3 max = glm::max(vec1, vec2);

  glm::vec3 pp = vehicle->getPosition();
...
void game_get_random_character_in_zone(const ScriptArguments& args)
{
  ...
  if (zfind != zones.end()) {
    ...
    for (auto& p : args.getWorld()->pedestrianPool.objects) {
      ...
      auto& min = zfind->second.min;
      auto& max = zfind->second.max;
      if (cp.x > min.x && cp.y > min.y && cp.z > min.z && cp.x < max.x && cp.y < max.y &&
          cp.z < max.z) {
...
void game_enter_as_passenger(const ScriptArguments& args)
{
  ...
  character->controller->setNextActivity(
      new Activities::EnterVehicle(vehicle, Activities::EnterVehicle::ANY_SEAT));
}
...
ObjectModule::ObjectModule() : ScriptModule("Object")
{
  ...
  bindFunction(0x00E6, game_locate_character_stopped_2d<PlayerController>, 6,
               "Locate Player Stopped 2D");

  bindFunction(0x00E8, game_locate_player_stopped_in_vehicle_2d, 6,
               "Locate Player Stopped in Vehicle 2D");
  bindFunction(0x00E9, game_character_near_character_2D, 5, "Locate Character near Character 2D");

  bindFunction(0x00EB, game_character_near_character_in_vehicle_2D<PlayerController>, 5,
               "Is player near character in vehicle");

  bindFunction(0x00ED, game_character_near_point_on_foot_2D, 6, "Is Character near point on foot");

  bindFunction(0x00EF, game_locate_character_stopped_2d<CharacterObject>, 6,
               "Locate Character Stopped 2D");
  ...
  bindFunction(0x00F9, game_player_stopped_near_point_on_foot_3d, 8,
               "Detect player stopped on foot area 3d");
  bindFunction(0x00FA, game_player_stopped_near_point_in_vehicle_3d, 8,
               "Detect player stopped in car area 3d");

  bindFunction(0x00FD, game_is_player_in_vehicle_near_character, 6,
               "Is Player in Vehicle Near Character");
  ...
  bindFunction(0x0199, game_character_in_area_or_cylinder<PlayerController>, 6,
               "Is Player in Area or Cylinder");

  bindFunction(0x019C, game_character_in_area_on_foot<PlayerController>, 8,
               "Is Player in Area on Foot");
  bindFunction(0x019E, game_character_stoped_in_volume<PlayerController>, 8,
               "Is Player stopped in volume");

  bindFunction(0x01A0, game_character_stoped_in_volume_in_vehicle<PlayerController>, 8,
               "Is Player Stopped in cube in vehicle");
  bindFunction(0x01A8, game_character_stoped_in_volume<CharacterObject>, 8,
               "Is Char Stopped in volume");
  bindFunction(0x01AA, game_character_stoped_in_volume_in_vehicle<CharacterObject>, 8,
               "Is Char Stopped in cube in vehicle");
  ...
  bindUnimplemented(0x01C2, game_mark_object_as_unneeded<CharacterObject>, 1,
                    "Mark Character Unneeded");
  bindUnimplemented(0x01C3, game_mark_object_as_unneeded<VehicleObject>, 1,
                    "Mark Vehicle Unneeded");
  ...
  bindUnimplemented(0x01CC, game_set_character_objective_kill_player_any_means, 2,
                    "Set Character Objective to Kill Player by Any Means");
  ...
  bindUnimplemented(0x0291, set_character_attack_when_provoked, 2,
                    "Set Character Attack When Provoked");
  ...
  bindUnimplemented(0x02AA, game_set_car_only_damageable_by_player, 2,
                    "Set Car Only Damageable by Player");
  ...
  bindFunction(0x034F, game_destroy_object_with_fade<CharacterObject>, 1,
               "Destroy Character with Fade");
  ...
  bindUnimplemented(0x0365, game_set_character_objective_to_hail_taxi, 1,
                    "Set Character Objective to Hail Taxi");
  ...
  bindUnimplemented(0x03BC, game_create_cylinder, 5, "Create Cylinder");  /// @todo Other sources
                                                                          /// call this "sphere",
                                                                          /// this should be our
                                                                          /// AreaIndicator?
  ...
  bindFunction(0x0443, game_player_in_any_vehicle, 1,
               "Is Player In Any Vehicle");  /// Duplicate of 00E0?!
  bindFunction(0x0448, game_character_in_vehicle<CharacterObject>, 2,
               "Is Character in This Vehicle");
}
