...
void drawMap(ViewCamera& currentView, PlayerController* player, GameWorld* world,
             GameRenderer* render)
{
...
{
  float infoTextX =
      render->getRenderer()->getViewport().x - (ui_outerMargin + ui_weaponSize + ui_infoMargin);
  float infoTextY = 0.f + ui_outerMargin;
  float iconX = render->getRenderer()->getViewport().x - (ui_outerMargin + ui_weaponSize);
  ...
  {
    std::stringstream ss;
    ss << std::setw(2) << std::setfill('0') << world->getHour() << std::setw(0) << ":"
  ...
  if (itemTexture != nullptr) {
    ...
    render->drawTexture(itemTexture.get(), glm::vec4(iconX, iconY, ui_weaponSize, ui_weaponSize));
  }
  ...
  if (current) {
    ...
    if (wep->getWeaponData()->fireType != WeaponData::MELEE) {
      ...
      ti.text = std::to_string(slotInfo.bulletsClip) + "-" + std::to_string(slotInfo.bulletsTotal);
...
void drawHUD(ViewCamera& currentView, PlayerController* player, GameWorld* world,
             GameRenderer* render)
{
