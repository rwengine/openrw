...
GameConfig::GameConfig(const std::string& configName, const std::string& configPath)
    : m_configName(configName), m_configPath(configPath), m_valid(false), m_inputInvertY(false)
{
...
std::string GameConfig::getConfigFile() { return m_configPath + "/" + m_configName; }
...
#elif defined(RW_OSX)
  char* home = getenv("HOME");
  if (home) return std::string(home) + "/Library/Preferences/" + kConfigDirectoryName;
...
int GameConfig::handler(void* user, const char* section, const char* name, const char* value)
{
