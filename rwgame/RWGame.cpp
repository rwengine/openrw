...
RWGame::RWGame(int argc, char* argv[])
{
  ...
  if (SDL_Init(SDL_INIT_VIDEO) < 0) throw std::runtime_error("Failed to initialize SDL2!");
...
void RWGame::saveGame(const std::string& savename) { RW_UNUSED(savename); }
...
void RWGame::render(float alpha, float time)
{
  ...
  if (state->currentCutscene != nullptr && state->cutsceneStartTime >= 0.f) {
    ...
    float cutsceneTime =
        std::min(world->getGameTime() - state->cutsceneStartTime, cutscene->tracks.duration);
...
void RWGame::renderDebugStats(float time, Renderer::ProfileInfo& worldRenderTime)
{
  ...
  ss << "Average (per " << average_every_frame << " frames); Frametime: " << time_average
     << " (FPS " << (1000.f / time_average) << ")\n";
  ...
  for (auto& perf : profGroups) {
    ss << "  " << perf.first << ": " << perf.second->draws << " draws " << perf.second->primitives
       << " prims " << (perf.second->duration / 1000000) << "ms\n";
  }
  ...
  for(auto it = engine->log.begin(); it != engine->log.end(); ++it) {
      ...
      // c.a = (engine->gameTime - it->time > 5.f) ? 255 - (((engine->gameTime - it->time) -
  5.f)/5.f) * 255 : 255;
      // text.setColor(c);
  ...
  std::function<void(const perf::ProfileEntry&, int)> renderEntry =
      [&](const perf::ProfileEntry& entry, int depth) {
        ...
        for (auto& event : entry.childProfiles) {
          ...
          renderer->drawColour(
              perf_colours[(std::hash<std::string>()(entry.label) * (g++)) % perf_colours.size()],
