...
static void jumpCharacter(RWGame* game, CharacterObject* player, const glm::vec3& target,
                          bool ground = true)
{
...
Menu* DebugState::createDebugMenu()
{
  ...
  m->addEntry(Menu::lambda("Jump to Debug Camera", [=] {
    jumpCharacter(game, player, _debugCam.position + _debugCam.rotation * glm::vec3(3.f, 0.f, 0.f),
                  false);
  }, kDebugEntryHeight));

  m->addEntry(Menu::lambda("-Map", [=] { this->enterMenu(createMapMenu()); }, kDebugEntryHeight));
  m->addEntry(
      Menu::lambda("-Vehicles", [=] { this->enterMenu(createVehicleMenu()); }, kDebugEntryHeight));
  m->addEntry(Menu::lambda("-AI", [=] { this->enterMenu(createAIMenu()); }, kDebugEntryHeight));
  m->addEntry(
      Menu::lambda("-Weapons", [=] { this->enterMenu(createWeaponMenu()); }, kDebugEntryHeight));

  m->addEntry(
      Menu::lambda("Set Super Jump", [=] { player->setJumpSpeed(20.f); }, kDebugEntryHeight));
  m->addEntry(Menu::lambda("Set Normal Jump", [=] {
    player->setJumpSpeed(CharacterObject::DefaultJumpSpeed);
  }, kDebugEntryHeight));
  m->addEntry(Menu::lambda("Full Health", [=] { player->getCurrentState().health = 100.f; },
                           kDebugEntryHeight));
  m->addEntry(Menu::lambda("Full Armour", [=] { player->getCurrentState().armour = 100.f; },
                           kDebugEntryHeight));
...
Menu* DebugState::createMapMenu()
{
  ...
  m->addEntry(Menu::lambda("Back", [=] { this->enterMenu(createDebugMenu()); }, kDebugEntryHeight));
  ...
  m->addEntry(Menu::lambda("Unsolid garage doors", [=] {
    ...
    std::vector<std::string> garageDoorModels{
        "8ballsuburbandoor",  "amcogaragedoor",     "bankjobdoor",        "bombdoor",
        "crushercrush",       "crushertop",         "door2_garage",       "door3_garage",
        "door4_garage",       "door_bombshop",      "door_col_compnd_01", "door_col_compnd_02",
        "door_col_compnd_03", "door_col_compnd_04", "door_col_compnd_05", "door_jmsgrage",
        "door_sfehousegrge",  "double_garage_dr",   "impex_door",         "impexpsubgrgdoor",
        "ind_plyrwoor",       "ind_slidedoor",      "jamesgrge_kb",       "leveldoor2",
        "oddjgaragdoor",      "plysve_gragedoor",   "SalvGarage",         "shedgaragedoor",
        "Sub_sprayshopdoor",  "towergaragedoor1",   "towergaragedoor2",   "towergaragedoor3",
        "vheistlocdoor"};

    auto gw = game->getWorld();
    for (auto& i : gw->instancePool.objects) {
      auto obj = static_cast<InstanceObject*>(i.second);
      if (std::find(garageDoorModels.begin(), garageDoorModels.end(), obj->model->name) !=
...
Menu* DebugState::createVehicleMenu()
{
  ...
  for (auto& e : kVehicleTypes) {
    m->addEntry(Menu::lambda(e.first, [=] { spawnVehicle(e.second); }, kDebugEntryHeight));
  }
...
Menu* DebugState::createAIMenu()
{
  ...
  const std::map<std::string, int> kPedTypes = {
      {"Triad", 12}, {"Cop", 1}, {"SWAT", 2}, {"FBI", 3}, {"Fireman", 6}, {"Construction", 74},
  };

  for (auto& e : kPedTypes) {
    m->addEntry(
        Menu::lambda(e.first + " Follower", [=] { spawnFollower(e.second); }, kDebugEntryHeight));
  }
  ...
  m->addEntry(Menu::lambda("Kill All Peds", [=] {
    for (auto& p : game->getWorld()->pedestrianPool.objects) {
      ...
      p.second->takeDamage({p.second->getPosition(), p.second->getPosition(), 100.f,
...
Menu* DebugState::createWeaponMenu()
{
  ...
  for (int i = 1; i < maxInventorySlots; ++i) {
    ...
    m->addEntry(Menu::lambda(name, [=] { giveItem(item); }, kDebugEntryHeight));
  }
...
void DebugState::printCameraDetails()
{
  std::cout << " " << _debugCam.position.x << " " << _debugCam.position.y << " "
            << _debugCam.position.z << " " << _debugCam.rotation.x << " " << _debugCam.rotation.y
            << " " << _debugCam.rotation.z << " " << _debugCam.rotation.w << std::endl;
}
