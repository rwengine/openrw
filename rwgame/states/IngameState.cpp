...
void IngameState::startTest()
{
  ...
  for (int i = 1; i < maxInventorySlots; ++i) {
    ...
    getWorld()->createPickup(itemspawn, item->getModelID(), PickupObject::OnStreet);
    itemspawn.x += 2.5f;
  }
...
void IngameState::tick(float dt)
{
  ...
  if (player && player->isInputEnabled()) {
    ...
    auto vehicle = (target->type() == GameObject::Character)
    ...
    if (vehicle) {
      ...
      if (autolookTimer <= 0.f && glm::abs(velocity) > kAutolookMinVelocity) {
        ...
        m_cameraAngles.x +=
            glm::sign(deltaYaw) * std::min(kMaxRotationRate * dt, glm::abs(deltaYaw));
        m_cameraAngles.y +=
            glm::sign(deltaPitch) * std::min(kMaxRotationRate * dt, glm::abs(deltaPitch));
      }
    }
    ...
    if (player->isInputEnabled()) {
      ...
      if (input.pressed(GameInputState::EnterExitVehicle)) {
        ...
        if (player->getCharacter()->getCurrentVehicle()) {
          player->exitVehicle();
        } else if (!player->isCurrentActivity(Activities::EnterVehicle::ActivityName)) {
          player->enterNearestVehicle();
        }
      } else if (glm::length2(movement) > 0.001f) {
        if (player->isCurrentActivity(Activities::EnterVehicle::ActivityName)) {
    ...
    if (ray.hasHit() && ray.m_closestHitFraction < 1.f) {
      cameraPosition =
          glm::vec3(ray.m_hitPointWorld.x(), ray.m_hitPointWorld.y(), ray.m_hitPointWorld.z());
      cameraPosition +=
  ...
  switch (event.type) {
    ...
    case SDL_MOUSEMOTION:
      if (game->hasFocus()) {
        ...
        m_cameraAngles.y =
            glm::clamp(m_cameraAngles.y, kCameraPitchLimit, glm::pi<float>() - kCameraPitchLimit);
      }
      break;
    default:
      break;
