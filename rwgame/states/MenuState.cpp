...
MenuState::MenuState(RWGame* game) : State(game) { enterMainMenu(); }
...
void MenuState::enterMainMenu()
{
  ...
  m->addEntry(Menu::lambda(
      "Test", [=] { StateManager::get().enter(new IngameState(game, true, "test")); }));
...
void MenuState::enterLoadMenu()
{
  ...
  for (SaveGameInfo& save : saves) {
    if (save.valid) {
      std::stringstream ss;
      ss << save.basicState.saveTime.year << "/" << save.basicState.saveTime.month << "/"
         << save.basicState.saveTime.day << " " << save.basicState.saveTime.hour << ":"
         << save.basicState.saveTime.minute << "    " << save.basicState.saveName;
...
void MenuState::enter() { getWindow().showCursor(); }
