...
void LoaderDFF::readGeometry(Model *model, const RWBStream &stream)
{
  ...
  geom->dbuff.setFaceType(geom->facetype == Model::Triangles ? GL_TRIANGLES : GL_TRIANGLE_STRIP);
  ...
  size_t icount =
      std::accumulate(geom->subgeom.begin(), geom->subgeom.end(), 0u,
...
void LoaderDFF::readBinMeshPLG(Model *model, const RWBStream &stream)
{
  ...
  model->geometries.back()->facetype = static_cast<Model::FaceType>(*(std::uint32_t *)data);
