...
public:
  ...
  uint32_t loopStart;   /// loop start, where looping would begin relative to audio file's position,
                        /// 0 for beginning of audio file
  uint32_t loopEnd;     /// where looping would end relative to audio file's position, -1 for end of
                        /// audio file
};
