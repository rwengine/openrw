...
TextureData::Handle getErrorTexture()
{
  ...
  if (errTexName == 0) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 2, 2, 0, GL_RGBA, GL_UNSIGNED_BYTE, gErrorTextureData);
...
{
  uint8_t* dataBase = reinterpret_cast<uint8_t*>(rootSection.raw() + sizeof(RW::BSSectionHeader) +
                                  ...
                                  RW::BinaryStreamSection& rootSection)
{
  ...
  bool isPal8 = (texNative.rasterformat & RW::BSTextureNative::FORMAT_EXT_PAL8) ==
  ...
  // Export this value
  bool transparent = !((texNative.rasterformat & RW::BSTextureNative::FORMAT_888) ==
  ...
  if (isPal8) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, texNative.width, texNative.height, 0, GL_RGBA,
                 GL_UNSIGNED_BYTE, fullColor.data());
  } else if (isFulc) {
    ...
    glGenTextures(1, &textureName);
    glBindTexture(GL_TEXTURE_2D, textureName);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, texNative.width, texNative.height, 0, format, type,
                 coldata);
  } else {
    return getErrorTexture();
  }
...
LoadTextureArchiveJob::LoadTextureArchiveJob(WorkContext* context, FileIndex* index,
                                             TextureArchive& inTextures, const std::string& file)
    : WorkJob(context), archive(inTextures), fileIndex(index), _file(file)
{
}

void LoadTextureArchiveJob::work() { data = fileIndex->openFile(_file); }
