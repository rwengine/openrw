...
public:
  ...
  RWBStream(char* data, size_t size) : _data(data), _size(size), _dataCur(data), _nextChunk(data) {}
  ...
  char* getCursor() const { return _dataCur; }

  size_t getCurrentChunkSize() const { return _currChunkSz; }

  std::uint32_t getChunkVersion() const { return _chunkVersion; }

  /**
   * @brief Returns a new stream for the data inside this one.
   */
  RWBStream getInnerStream() const { return RWBStream(_dataCur, _currChunkSz); }
};
...
struct BSTextureNative {
  ...
  enum { WRAP_NONE = 0x00, WRAP_WRAP = 0x01, WRAP_MIRROR = 0x02, WRAP_CLAMP = 0x03 };
...
public:
  ...
  char* raw() { return data + offset + sizeof(BSSectionHeader); }

  bool hasMoreData(size_t length) { return (length) < (header.size); }
