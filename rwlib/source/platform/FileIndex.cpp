...
/**
 * Finds the 'real' case for a path, to get around the fact that Rockstar's data is usually the
 * wrong case.
...
std::string findPathRealCase(const std::string& base_src, const std::string& path_src)
{
  ...
  for (const path& path_component :
       boost::make_iterator_range(searchpath.begin(), searchpath.end())) {
