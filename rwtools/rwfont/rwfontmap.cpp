...
const char* ft_error_to_string(int error) {
    switch (error) {
#define FT_NOERRORDEF_(_DC, CODE, STR)                                         \
    case CODE:                                                                 \
        return STR;
#define FT_ERRORDEF_(_DC, CODE, STR)                                           \
    case CODE:                                                                 \
        return STR;
...
[[noreturn]] void ft_error(int code) {
...
public:
    FontTextureBuffer(unsigned fontsize, unsigned width, unsigned height,
                      float height_width_ratio)
        : m_texture{static_cast<int>(width), static_cast<int>(height),
    ...
    void draw_glyph_mono(int index, FT_GlyphSlot glyph) {
        int x = index % 16;
        int y = index / 16 - 2;
        QPoint topleft = {x * static_cast<int>(m_glyph_width),
                          y * static_cast<int>(m_glyph_height)};
        QPoint baselineleft =
            topleft + QPoint{0, static_cast<int>(m_glyph_width)};

        if (baselineleft.x() + glyph->metrics.horiBearingX / 64 +
                glyph->metrics.width / 64 >=
            topleft.x() + m_glyph_width) {
        ...
        if (baselineleft.y() - glyph->metrics.horiBearingY / 64 +
                glyph->metrics.height / 64 >=
            topleft.y() + m_glyph_height) {
            ...
            for (unsigned i = 0; i < glyph->bitmap.width; ++i) {
                ...
                QPoint point =
                    ...
                    QPoint{static_cast<int>(i),
                           static_cast<int>(
                               row - (glyph->metrics.horiBearingY / 64))};
    ...
    void draw_glyph_normal(int index, FT_GlyphSlot glyph) {
        ...
        QPoint topleft = {x * static_cast<int>(m_glyph_width),
                          y * static_cast<int>(m_glyph_height)};
        QPoint baselineleft =
            topleft + QPoint{0, 4 * static_cast<int>(m_glyph_height) / 5};
    ...
    void create_font_map(const FontMap& fontmap, RenderMode mode) {
        ...
        for (auto it = fontmap.to_unicode_begin();
             it != fontmap.to_unicode_end(); ++it) {
            while (it->first != next) {
            ...
            for (auto face : m_faces) {
                ...
                int advance = face->glyph->metrics.horiAdvance /
                              64;  // - face->glyph->metrics.horiBearingX / 64;
    ...
    void write(const rwfs::path& bitmap_path, const rwfs::path& advance_path) {
        ...
        //        ofs.write(reinterpret_cast<const char *>(m_advances.data()),
        //        m_advances.size());
    }
...
int main(int argc, const char* argv[]) {
    ...
    desc.add_options()("help", "Show this help message")(
        "fontsize,s", po::value<unsigned>()->value_name("FONTSIZE")->required(),
        "Fontsize")("width,w",
                    po::value<unsigned>()->value_name("WIDTH")->required(),
                    "Width of the texture")(
        "height,h", po::value<unsigned>()->value_name("HEIGHT")->required(),
        "Height of the texture")("ratio,r",
                                 po::value<float>()->value_name("ASPECTRATIO"),
                                 "Aspect ratio")(
        "map,m", po::value<unsigned>()->value_name("MAP")->required(),
        "Font map to use")(
        "font,f",
        po::value<std::vector<std::string>>()->value_name("PATH")->required(),
        "Path to fonts")(
        "texture,t", po::value<rwfs::path>()->value_name("PATH")->required(),
        "Output texture")(
        "advance,a", po::value<rwfs::path>()->value_name("PATH")->required(),
        "Output advances");
    ...
    texBuffer.create_font_map(fontmaps_gta3_font[fontmap_index],
                              FontTextureBuffer::RenderMode::NORMAL);

    texBuffer.write(vm["texture"].as<rwfs::path>(),
                    vm["advance"].as<rwfs::path>());
    return 0;
}
