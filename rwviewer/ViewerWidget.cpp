...
ViewerWidget::ViewerWidget(QOpenGLContextWrapper* context, QWindow* parent)
    : QWindow(parent), m_context(context) {
...
void ViewerWidget::drawModel(GameRenderer& r, ClumpPtr& model) {
    ...
    const auto& vc =
        OrbitCamera({width(), height()}, viewAngles, viewDistance, view, proj);

    r.getRenderer()->useProgram(r.worldProg.get());
    r.getRenderer()->setSceneParameters({proj, view, glm::vec4(0.15f),
                                         glm::vec4(0.7f), glm::vec4(1.f),
